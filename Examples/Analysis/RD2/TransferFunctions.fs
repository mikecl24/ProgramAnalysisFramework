...
let rec genIotaV (vars, oldIota) =
    match vars with
    | [] -> oldIota
    | var::next -> genIotaV (next, (Set.union oldIota (Set.empty.Add({Union1 = Var1 var; Union2 = List1(QM); Q2 = Node(0)}))))


let rec genIotaA (arrs, oldIota) =
    match arrs with
    | [] -> oldIota
    | arr::next -> genIotaA (next, (Set.union oldIota (Set.empty.Add({Union1 = Arr1 arr; Union2 = List1(QM); Q2 = Node(0)}))))
...
let rec remove (inSet, outSet, killCond) = 
    match inSet with
    | [] -> outSet
    | x::xs when x.Union1=killCond -> remove (xs, outSet, killCond)
...
let TF_Assignment (inSigma : sigma, edge : Edge) : sigma = (Set.union (Set.empty.Add({Union1 = Var1 (getVar edge.Action); Union2 = Q1(edge.Q2);Q2 = edge.Q1})) (remove ((Set.toList inSigma), Set.empty, (Var1 (getVar edge.Action)))))

let TF_Skip (inSigma : sigma, edge : Edge) : sigma = inSigma

// May not kill in arrays
let TF_ArrayAssignment (inSigma : sigma, edge : Edge) : sigma = (Set.union (Set.empty.Add({Union1 = Arr1 (getArr edge.Action); Union2 = Q1(edge.Q2) ;Q2 = edge.Q1})) inSigma)