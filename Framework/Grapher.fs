...
let rec graphIt (startNode : int) (endNode : int) (actions : Statement list) (freshNext : int) (edges : Edge list) (ifStart : int list) (ifEnd : int list) (doNodes : int list) : Edge list = 
    ...
    | x::xs when x.s_type=S_VarAssignment   
        -> (graphIt endNode     freshNext   xs (freshNext+1)    ([{Q1 = Node(startNode); Q2 = Node(endNode); Action = x.commandAST; Type = Assignment}]@edges) ifStart ifEnd doNodes)
    
    | x::xs when x.s_type=S_Skip           
        -> (graphIt endNode     freshNext   xs (freshNext+1)    ([{Q1 = Node(startNode); Q2 = Node(endNode); Action = x.commandAST; Type = Skip}]@edges) ifStart ifEnd doNodes)
    
    | x::xs when x.s_type=S_IfBool          
        -> (graphIt endNode     freshNext   xs (freshNext+1)    ([{Q1 = Node(startNode); Q2 = Node(endNode); Action = x.commandAST; Type = Boolean}]@edges) (ifStart@[startNode]) ifEnd doNodes)
    
    | x::xs when x.s_type=S_IfElse          
        -> (graphIt endNode     freshNext   xs (freshNext+1)    ([{Q1 = Node(ifStart.Head); Q2 = Node(endNode); Action = x.commandAST; Type = Boolean}]@edges) (ifStart.Tail) ([startNode]@ifEnd) doNodes)
    ...
    | x::xs when x.s_type=S_DoBool          
        -> (graphIt endNode     freshNext   xs (freshNext+1)    ([{Q1 = Node(startNode); Q2 = Node(endNode); Action = x.commandAST; Type = Boolean}]@edges) ifStart ifEnd ([startNode]@doNodes))
    
    | x::xs when x.s_type=S_DoOd            
        -> (graphIt startNode   endNode     xs freshNext        ([{Q1 = Node(doNodes.Head); Q2 = Node(startNode); Action = x.commandAST; Type = Boolean}]@correctEdges edges edges.Head.Q2 (Node(doNodes.Head))) ifStart ifEnd doNodes.Tail)
    
    | x::xs when x.s_type=S_ArrAssignment   
        -> (graphIt endNode     freshNext   xs (freshNext+1)    ([{Q1 = Node(startNode); Q2 = Node(endNode); Action = x.commandAST; Type = ArrayAssignment}]@edges) ifStart ifEnd doNodes)
